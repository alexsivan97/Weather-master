...
- (void)creatTableView {
    ...
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width-15, self.bounds.size.height) style:UITableViewStylePlain];
...
//    NSLog(@"cell");
    if(indexPath.row == 0) {
        ...
        NSString *strc = [NSString stringWithFormat:@"地区：%@",_cityName];
        cell.cityLabel.text = strc;
        //yzy add part
        NSString *strw = [NSString stringWithFormat:@"%@(实时)",_nowDictionary[@"cond_txt"]];
        cell.weatherLabel.text = strw;
        NSString *strt = [NSString stringWithFormat:@"今日:%@~%@°C",_oneDayDictionary[@"tmp_min"],_oneDayDictionary[@"tmp_max"]];
        cell.tempLabel.text = strt;
        //cell.WeatherImageLabel.image = [UIImage imageNamed:_weekImageArray[indexPath.row]];
        //yzy add part
        NSDateFormatter *nsdf1 = [[NSDateFormatter alloc] init];
        [nsdf1 setDateStyle:NSDateFormatterShortStyle];
        [nsdf1 setDateFormat:@"HH"];
        NSString *date1 = [nsdf1 stringFromDate:[NSDate date]];
        int ivalue = [date1 intValue];
        cell.WeatherImageLabel.image = [UIImage imageNamed:_weatherImageArray[ivalue]];
        
        //yzy add part end
        /*yzy delete part
        cell.weatherLabel.text = _nowDictionary[@"cond_txt"]; */
        
        NSString *str = [NSString stringWithFormat:@"%@°C",_nowDictionary[@"tmp"]];
